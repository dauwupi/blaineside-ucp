...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .account-meta{display:flex;flex-direction:column;line-height:1.3;flex:1;min-width:0}
  .account-meta .u{font-size:13.5px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  ...
  .slide p{font-size:13px;color:#c9bea9;max-width:92%}
  ...
  .announce{display:flex;align-items:center;gap:14px;padding:14px 18px;
    background:linear-gradient(90deg,rgba(212,146,58,.14),rgba(212,146,58,.05));
    border:1px solid rgba(212,146,58,.34);border-radius:12px;position:relative}
  .announce-badge{display:inline-flex;align-items:center;gap:8px;flex:none;
    font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#1a1206;
    background:linear-gradient(145deg,var(--gold),var(--amber));padding:6px 11px;border-radius:100px}
  .announce-badge svg{width:14px;height:14px}
  .announce-text{flex:1;font-size:13.5px;color:#e6d3b2;line-height:1.5;text-align:center;
    padding:0 8px}
  .announce-text b{color:var(--parchment);font-weight:700}
  .announce-x{flex:none;width:28px;height:28px;display:grid;place-items:center;border-radius:8px;
    background:transparent;border:none;color:var(--text-faint);cursor:pointer;transition:.14s}
  .announce-x:hover{background:rgba(212,146,58,.14);color:var(--parchment)}
  .announce-x svg{width:15px;height:15px}

  /* NOTIFICATION BELL DROPDOWN */
  .notif{position:relative}
  .notif-menu{position:absolute;right:0;top:calc(100% + 12px);width:352px;
    background:var(--charcoal-2);border:1px solid var(--border);border-radius:14px;
    box-shadow:0 26px 54px -18px rgba(0,0,0,.82);z-index:60;overflow:hidden}
  .notif-head{display:flex;align-items:center;justify-content:space-between;padding:15px 16px 13px;
    border-bottom:1px solid var(--border-soft)}
  .nh-title{font-size:14px;font-weight:700;display:flex;align-items:center;gap:8px}
  .nh-count{font-size:11px;font-weight:700;color:#1a1206;background:var(--gold);padding:1px 8px;border-radius:100px}
  .nh-clear{background:none;border:none;font-family:inherit;font-size:12px;font-weight:600;
    color:var(--text-faint);cursor:pointer;transition:.13s}
  .nh-clear:hover{color:var(--gold)}
  .notif-list{max-height:360px;overflow-y:auto;padding:6px}
  .notif-item{display:flex;align-items:flex-start;gap:12px;padding:11px 12px;border-radius:10px;
    position:relative;transition:.13s}
  .notif-item:hover{background:var(--charcoal-3)}
  .notif-item.unread{background:rgba(212,146,58,.05)}
  .ni-ico{width:34px;height:34px;border-radius:9px;flex:none;display:grid;place-items:center;
    background:var(--charcoal-3);border:1px solid var(--border)}
  .ni-ico svg{width:16px;height:16px}
  .ni-ico.green{color:var(--ok)} .ni-ico.red{color:var(--danger)}
  .ni-ico.amber{color:var(--gold)} .ni-ico.blue{color:#7f9bd9} .ni-ico.slate{color:#a8afd4}
  .ni-body{flex:1;min-width:0}
  .ni-t{font-size:13px;font-weight:500;color:var(--parchment);line-height:1.45;display:block}
  .ni-t b{color:var(--gold);font-weight:700}
  .ni-time{font-size:11px;color:var(--text-dim);margin-top:3px;display:block}
  .ni-un{width:8px;height:8px;border-radius:50%;background:var(--gold);flex:none;margin-top:5px}
  .notif-foot{display:block;text-align:center;padding:13px;font-size:12.5px;font-weight:600;
    color:var(--text-faint);border-top:1px solid var(--border-soft);transition:.13s}
  .notif-foot:hover{color:var(--gold);background:var(--charcoal-3)}

  /* tick button (mark read) shared by dropdown + page */
  .ni-tick{flex:none;width:26px;height:26px;border-radius:7px;display:grid;place-items:center;
    background:transparent;border:1px solid var(--border);color:var(--text-dim);cursor:pointer;
    transition:.14s;margin-top:3px}
  .ni-tick svg{width:14px;height:14px;stroke-width:2.4}
  .ni-tick:hover{border-color:var(--ok);color:var(--ok);background:rgba(127,160,90,.1)}
  .notif-item.read .ni-tick,.np-row.read .ni-tick{border-color:var(--ok);color:var(--ok);
    background:rgba(127,160,90,.14);cursor:default}
  .notif-item.read .ni-t,.np-row.read .ni-t{color:var(--text-faint)}
  .notif-item.read .ni-t b,.np-row.read .ni-t b{color:var(--text-faint)}

  /* ===== FULL NOTIFICATIONS PAGE ===== */
  .page-back{display:inline-flex;align-items:center;gap:8px;font-size:13px;font-weight:600;
    color:var(--text-faint);margin-bottom:18px;transition:.14s}
  .page-back:hover{color:var(--parchment)}
  .page-back svg{width:16px;height:16px}
  .np-head{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;margin-bottom:16px}
  .np-title{font-size:22px;font-weight:700;letter-spacing:-.02em}
  .np-sub{font-size:13px;color:var(--text-faint);margin-top:3px}
  .np-clear{background:var(--charcoal-2);border:1px solid var(--border);border-radius:9px;
    padding:9px 14px;font-family:inherit;font-size:13px;font-weight:600;color:var(--text-faint);
    cursor:pointer;transition:.14s}
  .np-clear:hover{color:var(--parchment);background:var(--charcoal-3)}
  .np-filters{display:flex;gap:6px;margin-bottom:18px}
  .np-filter{background:var(--charcoal-2);border:1px solid var(--border);border-radius:100px;
    padding:8px 16px;font-family:inherit;font-size:13px;font-weight:600;color:var(--text-faint);
    cursor:pointer;transition:.14s}
  .np-filter:hover{color:var(--parchment)}
  .np-filter.active{background:var(--charcoal-3);color:var(--parchment);border-color:var(--charcoal-4)}
  .np-list{padding:6px}
  .np-row{display:flex;align-items:flex-start;gap:14px;padding:15px 14px;border-radius:11px;transition:.13s}
  .np-row + .np-row{border-top:1px solid var(--border-soft)}
  .np-row:hover{background:var(--charcoal-3)}
  .np-row.unread{background:rgba(212,146,58,.045)}
  .np-row .ni-ico{width:38px;height:38px}
  .np-row .ni-ico svg{width:17px;height:17px}
  .np-row .ni-body{flex:1;min-width:0}
  .np-row .ni-t{font-size:14px;font-weight:500;line-height:1.5}
  .np-row .ni-sub{font-size:12.5px;color:var(--text-dim);margin-top:2px;line-height:1.45}
  .np-row .ni-time{font-size:11.5px;color:var(--text-dim);margin-top:5px}
  .np-empty{padding:50px 20px;text-align:center;color:var(--text-dim);font-size:13.5px}

  /* pager — centered */
  .np-pager{display:flex;align-items:center;justify-content:center;gap:6px;margin-top:18px}
  .np-pager .pg{min-width:34px;height:34px;padding:0 10px;display:grid;place-items:center;border-radius:9px;
  ...
  .np-pager .pg:hover:not(:disabled){color:var(--parchment);background:var(--charcoal-3)}
  .np-pager .pg.on{background:var(--charcoal-3);color:var(--parchment);border-color:var(--charcoal-4)}
  .np-pager .pg:disabled{opacity:.4;cursor:not-allowed}
  .np-pager .pg svg{width:15px;height:15px;stroke-width:2.2}
  .np-pager .pg-gap{color:var(--text-dim);padding:0 2px;font-size:13px}

  @media (max-width:760px){
    .np-head{flex-direction:column;align-items:flex-start;gap:10px}
  }

  @media (max-width:760px){
    .notif-menu{position:fixed;left:10px;right:10px;top:60px;width:auto;
      background:var(--charcoal-2);max-height:calc(100dvh - 80px);display:flex;flex-direction:column}
    .notif-menu .notif-list{flex:1;max-height:none}
    .notif-menu .notif-foot{background:var(--charcoal-2)}
    .announce{flex-wrap:wrap;gap:10px;padding:13px 14px 13px}
    .announce-text{flex-basis:100%;order:3;font-size:13px}
    .announce-x{position:absolute;top:11px;right:11px}
  }
...
<body>
  ...
  <!-- ================= SIDEBAR (rendered from config below) ================= -->
  <aside class="sidebar">
  ...
  <div class="main">
    ...
    <header class="topbar">
      ...
      <button class="icon-btn search-mini" title="Search" aria-label="Search">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4-4"/></svg>
      </button>
      <div class="notif">
        <button class="icon-btn" id="bellBtn" title="Notifications">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M6 9a6 6 0 0 1 12 0c0 5 2 6 2 6H4s2-1 2-6"/><path d="M10 20a2 2 0 0 0 4 0"/></svg>
          <span class="dot"></span>
        </button>
        <div class="notif-menu" id="notifMenu">
          <div class="notif-head">
            <span class="nh-title">Notifications <span class="nh-count" id="nhCount">3</span></span>
            <button class="nh-clear" id="nhClear">Mark all read</button>
          </div>
          <div class="notif-list" id="notifList"></div>
          <a class="notif-foot" href="#/notifications" id="viewAllNotifs">View all notifications</a>
        </div>
      </div>
      <div class="divider"></div>
      <!-- ACCOUNT — rank name shown under the username (in gold).
           admin_rank in the database (0–9); Members (0) show no tag:
      ...
      <div class="account">
        <div class="account-btn">
          ...
          <span class="account-meta">
            <span class="u" id="acctName">dustin_hale</span>
            <span class="r" id="acctRole">Senior Admin</span>
          </span>
          <svg class="caret" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M6 9l6 6 6-6"/></svg>
        </div>
        <div class="account-menu">
          <div class="mhead">
            <div class="n" id="menuName">dustin_hale</div>
            <div class="rr" id="menuRole">Senior Admin</div>
          </div>
          <div class="menu-item">
          ...
          <div class="menu-item">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.6 1.6 0 0 0 .3 1.8 2 2 0 1 1-2.8 2.8 1.6 1.6 0 0 0-2.7 1.1V21a2 2 0 0 1-4 0 1.6 1.6 0 0 0-2.7-1.1 2 2 0 1 1-2.8-2.8A1.6 1.6 0 0 0 4.6 15a2 2 0 0 1 0-4 1.6 1.6 0 0 0 1.1-2.7 2 2 0 1 1 2.8-2.8A1.6 1.6 0 0 0 12 4.6a2 2 0 0 1 4 0 1.6 1.6 0 0 0 2.7 1.1"/></svg>
            Settings
          </div>
    ...
    <main class="content">
      ...
      <!-- ANNOUNCEMENT BANNER — issued to players by staff. Design placeholder. -->
      <div class="announce" id="announce">
        <span class="announce-badge">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 11l14-5v12L3 15v-4zM17 8a3 3 0 0 1 0 8M6 15v3a1 1 0 0 0 1 1h2"/></svg>
          Announcement
        </span>
        <div class="announce-text">
          <b>Scheduled maintenance this Sunday, 03:00 UTC.</b> The server and UCP will be offline for roughly two hours while we roll out the 3.2 update.
        </div>
        <button class="announce-x" aria-label="Dismiss">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 6l12 12M18 6L6 18"/></svg>
        </button>
      </div>

      <div class="hello">
        <h2>Welcome back, <b id="welcomeName">dustin_hale</b></h2>
      ...
      <div class="stats" id="statStrip">
        <div class="stat">
          ...
          <div class="num" data-count="42318">42,318</div>
        </div>
        <div class="stat">
        ...
        <div class="stat">
          ...
          <div class="num" data-count="126">126</div>
        </div>
        <div class="stat">
        ...
        <div class="col">
          <div class="panel" style="flex:1">
            <div class="panel-head">
              ...
              <a class="link" href="#/bulletin">All news →</a>
            </div>
        ...
        <div class="col">
          ...
          <div class="panel" style="flex:1">
            ...
            <div class="status-list">
              <div class="status-row up"><span class="led"></span><span class="nm">Game server</span><span class="val">Operational</span><span class="sub">142 online</span></div>
              <div class="status-row up"><span class="led"></span><span class="nm">Forums</span><span class="val">Operational</span><span class="sub">99.9% up</span></div>
              <div class="status-row up"><span class="led"></span><span class="nm">UCP</span><span class="val">Operational</span><span class="sub">4ms</span></div>
              <div class="status-row deg"><span class="led"></span><span class="nm">Discord</span><span class="val">Degraded</span><span class="sub">voice lag</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- ===== NOTIFICATIONS PAGE ===== -->
      <div class="page" id="page-notifications" style="display:none">
        <a class="page-back" id="notifBack" href="#/">
        ...
        <div class="np-head">
          <div>
            ...
            <p class="np-sub"><span id="npUnreadCount">3</span> unread of <span id="npTotalCount">5</span></p>
          </div>
          <button class="np-clear" id="npClear">Mark all read</button>
        </div>
        <div class="np-filters" id="npFilters">
          <button class="np-filter active" data-filter="all">All</button>
          <button class="np-filter" data-filter="unread">Unread</button>
          <button class="np-filter" data-filter="read">Read</button>
        </div>
        <div class="panel">
          <div class="np-list" id="npList"></div>
        </div>
        <div class="np-pager" id="npPager"></div>
      </div>
...
<script>
  /* =====================================================================
     ...
     ===================================================================== */
  let UCP_NAME = 'dustin_hale';
  let UCP_RANK = 6;                 // 0–9 (see ladder below); 6 = Senior Admin
  let UCP_ROLE = 'Senior Admin';   // display name; '' for Members (rank 0)
    ...
    fetch('api/session.php',{credentials:'include'})
      .then(r=>r.json()).then(d=>{
        if(d && d.ok && d.authenticated){
          ...
          UCP_ROLE = d.role || '';   // server already maps rank→name ('' for Members)
          applyIdentity();
  ...
  function applyIdentity(){
    // first name for the friendly greeting, full UCP name everywhere else
    const set=(id,val)=>{const el=document.getElementById(id); if(el) el.textContent=val;};
    set('welcomeName', UCP_NAME);
    set('acctName', UCP_NAME);
    set('menuName', UCP_NAME);
    // role: hide the line entirely for players (no rank)
    const showRole = !!UCP_ROLE;
    ['acctRole','menuRole'].forEach(id=>{
      const el=document.getElementById(id);
      if(!el) return;
      el.textContent = UCP_ROLE || '';
      el.style.display = showRole ? '' : 'none';
    });
  }
  ...
  const SIDEBAR = [
    ...
    { label:'Characters', icon:'user', children:[
      // Synced with the server later: filled slots show the character's name,
      // unused slots render as "Empty character slot" (set empty:true).
      { label:'Marcus Webb', href:'#' },
    ...
    { label:'Reports & Appeals', icon:'doc',   href:'#' },
  ...
  function renderSidebar(items){
    ...
    nav.innerHTML = items.map(it=>{
      if(it.heading) return `<div class="nav-heading">${it.heading}</div>`;

      if(it.children){
        const subs = it.children.map(c=> c.empty
          ? `<a class="slot-empty">${c.label}</a>`
          : `<a href="${c.href||'#'}">${c.label}</a>`).join('');
        return `<div class="nav-group" data-collapsible>
            <div class="nav-item">
              ${svg(it.icon,'i')}<span class="lbl">${it.label}</span>${svg('chev','chev')}
            </div>
      ...
      return `<div class="nav-group">
          <div class="nav-item ${it.active?'active':''}">
            ${svg(it.icon,'i')}<span class="lbl">${it.label}</span>
          </div>
        </div>`;
    }).join('');

    nav.querySelectorAll('.nav-group[data-collapsible] > .nav-item').forEach(item=>{
  ...
  /* =====================================================================
     COUNTY BULLETIN  —  edit this array to add/remove bulletin posts.
     Newest first. `type` sets the tag colour: 'event' | 'update' | 'notice'.
     `theme` picks the background wash (1–3). `link` (optional) makes the whole
     slide clickable — clicking opens that URL in a new tab. Managed on the
     County Bulletin page; "All news →" links there.
     ===================================================================== */
  /* This is the set of bulletins marked "show on dashboard" (up to 5).
     On the live server this is fed by the County Bulletin page's shown:true items. */
  const BULLETIN = [
    { type:'event',  theme:1, title:"Sandy Shores Rodeo returns this weekend",
      body:"Sign-ups are open for the county's biggest live event — prizes, property giveaways and a full arena build.",
      by:"cody_barrett", when:"2 hours ago", link:"https://forums.blaineside.com/rodeo" },
    { type:'update', theme:2, title:"Server 3.2 — economy rebalance is live",
      body:"Paycheck tiers, fuel pricing and business payouts have been retuned from last month's feedback.",
      by:"dev_marcus", when:"Yesterday", link:"https://forums.blaineside.com/changelog/3-2" },
    { type:'notice', theme:3, title:"Whitelist applications reopen Monday",
      body:"New illegal faction slots are opening. Read the updated guidelines before intake closes.",
      by:"faction_lead", when:"3 days ago", link:null },
    { type:'event',  theme:1, title:"Founders Day parade down Route 68",
      body:"Join the whole county for a slow cruise and street party. Classic cars welcome — sign up in advance.",
      by:"events_kara", when:"4 days ago", link:"https://forums.blaineside.com/foundersday" },
    { type:'update', theme:2, title:"Property interior updates rolling out",
      body:"A fresh batch of interiors is being added to existing properties across the map this week.",
      by:"dev_marcus", when:"5 days ago", link:null },
  ];
  ...
  function renderBulletin(posts){
    ...
    track.innerHTML = posts.map((p,i)=>{
      ...
      return `
      <div class="slide s${p.theme||1} ${i===0?'on':''}${p.link?' has-link':''}"${linkAttr}>
        <div class="bg"></div>
        <div class="cap">
          <span class="tag ${TAGCLASS[p.type]||''}">${TAGLABEL[p.type]||'Notice'}</span>
          <h4>${p.title}</h4>
          <p>${p.body}</p>
          <div class="meta">${p.when} · ${p.by}${linkChip}</div>
        </div>
      </div>`;}).join('');
    dotsWrap.innerHTML = posts.map((_,i)=>`<span class="d ${i===0?'on':''}"></span>`).join('');
    /* open link on click / Enter — only slides that actually have a link */
  ...
  /* bulletin rotation with manual arrows */
  const { slides, dots } = renderBulletin(BULLETIN);
  let idx=0, newsTimer=null;
  function go(n){
    const len=slides.length; n=(n+len)%len;
    slides.forEach((s,i)=>s.classList.toggle('on',i===n));
    dots.forEach((d,i)=>d.classList.toggle('on',i===n));
    idx=n;
  }
  function startNews(){ if(slides.length>1){clearInterval(newsTimer);newsTimer=setInterval(()=>go(idx+1),10000);} }
  function nudge(n){ go(idx+n); startNews(); } /* reset timer on manual move */
  ...
  /* =====================================================================
     NOTIFICATIONS  —  edit this array. Shared by the header dropdown and
     the full notifications page. `read:false` shows as unread (gold dot +
     tint); the tick button marks an item read. `icon` + `tone` set the
     left glyph; `sub` (optional) shows extra detail on the full page.
     ===================================================================== */
  let NOTIFS = [
    { id:1, tone:'green', icon:'check',  read:false, when:'14 minutes ago',
      text:'<b>Marcus Webb</b> was approved', sub:'Your legal faction application was accepted.' },
    { id:2, tone:'red',   icon:'alert',  read:false, when:'1 hour ago',
      text:'Report <b>#4821</b> needs your reply', sub:'An admin has requested more information.' },
    { id:3, tone:'amber', icon:'house',  read:false, when:'3 hours ago',
      text:'Property purchase confirmed — <b>Grapeseed Ave 14</b>', sub:'$184,000 was deducted from your bank.' },
    { id:4, tone:'blue',  icon:'clock',  read:true,  when:'Yesterday',
      text:'Weekly paycheck deposited', sub:'Salary plus faction bonus.' },
    { id:5, tone:'slate', icon:'chat',   read:true,  when:'Yesterday',
      text:'New reply in <b>Faction Applications</b>', sub:'Faction Management responded to your thread.' },
    { id:6, tone:'amber', icon:'house',  read:true,  when:'2 days ago',
      text:'Rent collected on <b>Paleto Blvd 7</b>', sub:'$3,200 added to your business account.' },
    { id:7, tone:'green', icon:'check',  read:true,  when:'2 days ago',
      text:'Character <b>Elena Ruiz</b> name change approved', sub:'Your requested change is now live.' },
    { id:8, tone:'red',   icon:'alert',  read:true,  when:'3 days ago',
      text:'Appeal <b>#3390</b> was closed', sub:'Reviewed by Senior Admin — no action taken.' },
    { id:9, tone:'blue',  icon:'clock',  read:true,  when:'4 days ago',
      text:'Vehicle insurance renewed', sub:'Coverage extended for 30 days.' },
    { id:10, tone:'slate',icon:'chat',   read:true,  when:'5 days ago',
      text:'You were mentioned in <b>Court Records</b>', sub:'A new case references your character.' },
    { id:11, tone:'amber',icon:'house',  read:true,  when:'6 days ago',
      text:'Property tax due on <b>Grapeseed Ave 14</b>', sub:'Payment window closes in 3 days.' },
    { id:12, tone:'green', icon:'check', read:true,  when:'1 week ago',
      text:'Business permit renewed', sub:'Your license is valid for another quarter.' },
  ];
  /* Single notification type — one icon (bell) and one tone for every item. */
  const NOTIF_ICON='<path d="M6 9a6 6 0 0 1 12 0c0 5 2 6 2 6H4s2-1 2-6"/><path d="M10 20a2 2 0 0 0 4 0"/>';
  const tickSVG='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M20 6L9 17l-5-5"/></svg>';

  let npFilter='all', npPage=1;
  const NP_PER_PAGE=8; /* rows per page on the full notifications page */

  function unreadCount(){ return NOTIFS.filter(n=>!n.read).length; }

  function rowHTML(n){
    return `
      <div class="np-row ${n.read?'read':'unread'}" data-id="${n.id}">
        <span class="ni-ico amber"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">${NOTIF_ICON}</svg></span>
        <span class="ni-body">
          <span class="ni-t">${n.text}</span>
          ${n.sub?`<span class="ni-sub">${n.sub}</span>`:''}
          <span class="ni-time">${n.when}</span>
        </span>
        <button class="ni-tick" data-tick="${n.id}" title="${n.read?'Read':'Mark as read'}" ${n.read?'disabled':''}>${tickSVG}</button>
      </div>`;
  }

  function renderPager(totalPages){
    const pager=document.getElementById('npPager');
    if(totalPages<=1){ pager.innerHTML=''; return; }
    const prev=`<button class="pg" data-pg="${npPage-1}" ${npPage===1?'disabled':''} aria-label="Previous"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M15 18l-6-6 6-6"/></svg></button>`;
    const next=`<button class="pg" data-pg="${npPage+1}" ${npPage===totalPages?'disabled':''} aria-label="Next"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M9 6l6 6-6 6"/></svg></button>`;
    /* build page numbers with gaps for large counts */
    const nums=[]; const add=p=>nums.push(`<button class="pg ${p===npPage?'on':''}" data-pg="${p}">${p}</button>`);
    const gap=()=>nums.push('<span class="pg-gap">…</span>');
    const pages=[];
    for(let i=1;i<=totalPages;i++){
      if(i===1||i===totalPages||Math.abs(i-npPage)<=1) pages.push(i);
    }
    let last=0;
    pages.forEach(p=>{ if(p-last>1) gap(); add(p); last=p; });
    pager.innerHTML = prev + nums.join('') + next;
  }

  function renderNotifications(){
    const u=unreadCount();
    /* header bell dot + counts */
    document.querySelector('.notif .dot').style.display = u ? 'block' : 'none';
    const nhCount=document.getElementById('nhCount');
    nhCount.textContent=u; nhCount.style.display=u?'inline-block':'none';

    /* dropdown: newest 5 (array is authored newest-first) */
    const list=document.getElementById('notifList');
    list.innerHTML = NOTIFS.slice(0,5).map(n=>`
      <div class="notif-item ${n.read?'read':'unread'}" data-id="${n.id}">
        <span class="ni-ico amber"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">${NOTIF_ICON}</svg></span>
        <span class="ni-body"><span class="ni-t">${n.text}</span><span class="ni-time">${n.when}</span></span>
        <button class="ni-tick" data-tick="${n.id}" title="${n.read?'Read':'Mark as read'}" ${n.read?'disabled':''}>${tickSVG}</button>
      </div>`).join('');

    /* full page — newest first, paginated */
    const total=NOTIFS.length;
    document.getElementById('npUnreadCount').textContent=u;
    document.getElementById('npTotalCount').textContent=total;
    const filtered = NOTIFS.filter(n=> npFilter==='all' ? true : npFilter==='unread' ? !n.read : n.read);

    const totalPages=Math.max(1,Math.ceil(filtered.length/NP_PER_PAGE));
    if(npPage>totalPages) npPage=totalPages;
    const start=(npPage-1)*NP_PER_PAGE;
    const pageItems=filtered.slice(start,start+NP_PER_PAGE);

    const np=document.getElementById('npList');
    np.innerHTML = pageItems.length ? pageItems.map(rowHTML).join('')
      : `<div class="np-empty">No ${npFilter==='all'?'':npFilter+' '}notifications.</div>`;
    renderPager(totalPages);
  }

  function markRead(id){ const n=NOTIFS.find(x=>x.id===id); if(n){n.read=true;renderNotifications();} }
  function markAllRead(){ NOTIFS.forEach(n=>n.read=true); renderNotifications(); }

  /* tick clicks — capture phase so clicks inside the dropdown (which stops
     propagation to stay open) still reach this handler */
  document.addEventListener('click',e=>{
    const t=e.target.closest('[data-tick]');
    if(t){ e.preventDefault(); e.stopPropagation();
      t.classList.add('just-read');
      setTimeout(()=>t.classList.remove('just-read'),340);
      markRead(parseInt(t.dataset.tick,10));
    }
  },true);
  document.getElementById('nhClear').addEventListener('click',e=>{e.stopPropagation();markAllRead();});
  document.getElementById('npClear').addEventListener('click',markAllRead);

  /* page filters — reset to page 1 */
  document.getElementById('npFilters').addEventListener('click',e=>{
    const f=e.target.closest('.np-filter'); if(!f)return;
    npFilter=f.dataset.filter; npPage=1;
    document.querySelectorAll('.np-filter').forEach(x=>x.classList.toggle('active',x===f));
    renderNotifications();
  });

  /* pager clicks */
  document.getElementById('npPager').addEventListener('click',e=>{
    const p=e.target.closest('[data-pg]'); if(!p)return;
    npPage=parseInt(p.dataset.pg,10); renderNotifications();
    window.scrollTo({top:0,behavior:'smooth'});
  });

  renderNotifications();

  /* ---- simple routing: dashboard <-> notifications ---- */
  function showPage(name){
    document.getElementById('page-dashboard').style.display   = name==='notifications' ? 'none' : '';
    document.getElementById('page-notifications').style.display = name==='notifications' ? 'block' : 'none';
    document.querySelector('.page-title h1').textContent = name==='notifications' ? 'Notifications' : 'Dashboard';
    document.querySelector('.content').scrollTop=0; window.scrollTo(0,0);
  }
  document.getElementById('viewAllNotifs').addEventListener('click',e=>{e.preventDefault();closeNotif();showPage('notifications');});
  document.getElementById('notifBack').addEventListener('click',e=>{e.preventDefault();showPage('dashboard');});

  /* account menu */
  const accBtn=document.querySelector('.account-btn'), accMenu=document.querySelector('.account-menu');
  accMenu.style.display='none';
  accBtn.addEventListener('click',e=>{e.stopPropagation();closeNotif();accMenu.style.display=accMenu.style.display==='none'?'block':'none';});
  document.addEventListener('click',()=>{accMenu.style.display='none';closeNotif();});

  /* notification bell dropdown */
  const bellBtn=document.getElementById('bellBtn'), notifMenu=document.getElementById('notifMenu');
  notifMenu.style.display='none';
  function closeNotif(){notifMenu.style.display='none';}
  bellBtn.addEventListener('click',e=>{e.stopPropagation();accMenu.style.display='none';notifMenu.style.display=notifMenu.style.display==='none'?'block':'none';});
  notifMenu.addEventListener('click',e=>e.stopPropagation());

  /* announcement banner dismiss (design only) */
  const announce=document.getElementById('announce');
  announce&&announce.querySelector('.announce-x').addEventListener('click',()=>announce.style.display='none');
  ...
  /* UTC clock — ticks every second (sidebar footer) */
  ...
  function tickClock(){
    ...
    const te=document.getElementById('utcTime'), de=document.getElementById('utcDate');
  ...
  /* demo timing — remove/replace with your real data callback */
  startLoading();
