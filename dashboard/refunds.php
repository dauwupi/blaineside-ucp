...
<script>
  ...
  /* =====================================================================
     ...
     Three things this page will not do:
       ...
       - draw a form that cannot submit,
       - draw an empty list that reads as "you have none",
       - decide for itself who may open the staff panel.

     The last one matters most. api/queues.php answers it, this page asks,
     and when the answer is no it says who the view IS for rather than
     pretending the link was never there. Somebody who typed the URL
     deserves a sentence, not a blank page.
     ===================================================================== */
  var AREA  = "refunds";
  var VIEWS = [
  {
    ...
    "what": "Tell staff what you lost and how.",
    "steps": [
      "What was lost, and roughly what it was worth.",
      "When it happened, and what you were doing at the time.",
      "Evidence — a screenshot, a clip, or the name of anyone who saw it."
    ],
    "after": "It joins the refund queue and an administrator picks it up. You are told the outcome either way, with a reason."
  },
  {
    "key": "mine",
    "label": "My Requests",
    "what": "Everything you have sent, and where each one got to.",
    "steps": [
      "Open, being looked at, approved, or rejected.",
      "What the reviewing administrator wrote when they decided.",
      "What was actually returned to you, if anything."
    ],
    "after": "Nothing here is editable — a request is a record of what you said at the time."
  },
  {
    "key": "panel",
    "label": "Refund Request Panel",
    "what": "The queue, from the staff side.",
    "steps": [
      "Open a request, read the evidence, and see the player’s history.",
      "Approve, reject, or send it back asking for more detail.",
      "Every decision is recorded against the administrator who made it."
    ],
    "after": "Refunds move money and property, which is why this sits at Trainee Admin rather than with the wider staff team."
  }
];
  var STATE = null;                      // api/queues.php, for this area
  var VIEW  = VIEWS[0].key;

  function vBy(k){ for(var i=0;i<VIEWS.length;i++) if(VIEWS[i].key === k) return VIEWS[i]; return null; }

  var QI = {
    lock:'<rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V8a4 4 0 0 1 8 0v3"/>',
    build:'<path d="M14.7 6.3a4 4 0 0 1-5.4 5.4L4 17v3h3l5.3-5.3a4 4 0 0 0 5.4-5.4l-2.1 2.1-2.3-2.3z"/>',
    chev:'<path d="M9 6l6 6-6 6"/>'
  };
  function qsvg(n){ return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor">'+QI[n]+'</svg>'; }

  /* ---- the view switcher ----
     Rebuilt whenever the server answer lands, because a view the caller may
     not open is not drawn as a disabled tab — it is not a tab. The tab bar
     is navigation; the refusal belongs on the page they asked for, where it
     can explain itself. */
  function renderTabs(){
    var host = document.getElementById('qtabs');
    host.innerHTML = VIEWS.filter(function(v){
      var st = STATE && STATE.views ? STATE.views[v.key] : null;
      return !st || st.may || v.key === VIEW;      // keep the one being refused
    }).map(function(v){
      var st = STATE && STATE.views ? STATE.views[v.key] : null;
      var no = st && !st.may;
      return '<button class="qtab" role="tab" data-view="'+v.key+'" '+
             'aria-selected="'+(v.key === VIEW ? 'true' : 'false')+'">'+
             (no ? '<span class="lk">'+qsvg('lock')+'</span>' : '') +
  ...
  function renderView(){
    var v = vBy(VIEW), host = document.getElementById('qbody');
    if(!v) return;
    document.querySelector('.page-title h1').textContent = v.label;
    document.title = 'BlaineSide \u2014 ' + v.label;

    var st = STATE && STATE.views ? STATE.views[v.key] : null;

    /* Refused. Say who it is for, in the server's words — the page does not
       get to invent a rule the endpoint doesn't have. */
    if(st && !st.may){
      host.innerHTML =
        '<div class="card"><div class="card-b" style="padding-top:18px">' +
          '<div class="qno"><span class="si">'+qsvg('lock')+'</span><div>' +
            '<h4>'+escapeHtml(v.label)+' isn\u2019t open to you</h4>' +
            '<p>'+escapeHtml(st.why || '')+'</p>' +
            '<p>If you think that is wrong, speak to Staff Management rather than the ' +
            'administrator who sent you here.</p>' +
          '</div></div>' +
        '</div></div>';
      return;
    }

    var steps = v.steps.map(function(t, i){
      return '<li><span class="n">'+(i+1)+'</span><span>'+escapeHtml(t)+'</span></li>';
    }).join('');

    host.innerHTML =
      '<div class="qgrid">' +
        '<div class="card"><div class="card-h"><h3>'+escapeHtml(v.label)+'</h3></div>' +
          '<div class="card-b">' +
            '<p class="qwhat">'+escapeHtml(v.what)+'</p>' +
            '<ul class="qsteps">'+steps+'</ul>' +
            '<div class="qafter">'+escapeHtml(v.after)+'</div>' +
          '</div></div>' +
        '<div>' +
          '<div class="qstate"><span class="si">'+qsvg('build')+'</span><div>' +
            '<h4>Not switched on yet</h4>' +
            '<p>'+escapeHtml((STATE && STATE.why) || 'This isn\u2019t switched on yet.')+' ' +
            'Nothing is stored and nothing is sent from this page. Until it opens, ' +
            'raise it with staff the way you do now.</p>' +
          '</div></div>' +
          '<div class="card" style="margin-top:20px"><div class="card-h"><h3>Who this is for</h3></div>' +
            '<div class="card-b"><div class="qmeta">'+
              VIEWS.map(function(x){
                var s2 = STATE && STATE.views ? STATE.views[x.key] : null;
                var may = !s2 || s2.may;
                return '<div class="row"><span class="k">'+escapeHtml(x.label)+'</span>' +
                       '<span class="v'+(may ? '' : ' off')+'">'+(may ? 'You' : 'Not you')+'</span></div>';
              }).join('') +
            '</div></div></div>' +
        '</div>' +
      '</div>';
  ...
  /* The hash IS the view: the sidebar links to /dashboard/refunds#panel, so
     arriving, switching tabs and using the back button all go through one
     path rather than three that can disagree. */
  function go(key, push){
    if(!vBy(key)) key = VIEWS[0].key;
    VIEW = key;
  ...
  UCP.get('session.php').then(function(d){
    if(!d || d.authenticated !== true){
      window.location.replace('/login?return='+encodeURIComponent('/dashboard/refunds'));
      return;
    }
    ...
    /* The gate. Asked for, not assumed from the rank this page just read —
       Staff Management opens the Staff Report Panel below Management, and
       that rule lives in api/_queues.php, not here. */
    return UCP.get('queues.php').then(function(q){
      if(q && q.ok === true && q.queues){
        for(var i=0;i<q.queues.length;i++) if(q.queues[i].key === AREA) STATE = q.queues[i];
      }
      go((location.hash || '').replace('#',''), false);
    });
  }).catch(function(){
    /* No answer from the server. Draw the page with no gate resolved rather
       than a spinner that never ends — every view still says it isn't built,
       which is the true part, and the panel gate is enforced server-side
       whatever this page decided to draw. */
    go((location.hash || '').replace('#',''), false);
