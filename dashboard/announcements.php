...
?>
      ...
      <div class="view active" id="view-list">
        ...
        <div class="slotbar" id="liveBar" style="display:block;padding:0;border:0;background:none"></div>

        <div id="alist" style="margin-top:18px"></div>
        <div class="pager" id="pager"></div>
      </div>

      <!-- ============ EDITOR ============ -->
      <div class="view" id="view-edit">
        <a class="page-back" href="#" onclick="closeEditor();return false;"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M15 18l-6-6 6-6"/></svg>Back to announcements</a>
          ...
          <div class="form-card">
            <div class="field">
              ...
              <div class="seg" id="typeSeg">
                <button data-type="notice" class="on"><span class="swatch" style="background:var(--gold)"></span>Notice</button>
                <button data-type="maintenance"><span class="swatch" style="background:#6f9bcc"></span>Maintenance</button>
                <button data-type="warning"><span class="swatch" style="background:var(--amber)"></span>Warning</button>
                <button data-type="critical"><span class="swatch" style="background:var(--danger)"></span>Critical</button>
                <button data-type="success"><span class="swatch" style="background:var(--ok)"></span>Resolved</button>
              </div>
            </div>

            <div class="field">
              <label><span class="lt">Headline <span class="req">*</span></span><span class="count"><span id="leadCount">0</span>/120</span></label>
              <input type="text" id="fLead" maxlength="120" placeholder="e.g. Scheduled maintenance this Sunday, 03:00 UTC." oninput="syncPreview()">
              <span class="field-hint">Shown in bold, first. Lead with the fact.</span>
            </div>

            <div class="field">
              <label><span class="lt">Detail</span><span class="count"><span id="bodyCount">0</span>/240</span></label>
              <textarea id="fBody" maxlength="240" placeholder="The server and UCP will be offline for roughly two hours while we roll out the 3.2 update." oninput="syncPreview()"></textarea>
            </div>

            <div class="field">
              <label><span class="lt">Link</span></label>
              <input type="text" id="fLink" placeholder="https://… — optional, makes the strip clickable" oninput="syncPreview()">
              <span class="field-hint" id="linkHint"></span>
            </div>

            <label class="switchrow" id="dismissRow" onclick="toggleDismiss()">
              <span class="sw"></span>
              <span id="dismissLabel">Players can dismiss this</span>
            </label>
            <span class="field-hint" style="margin-top:8px;display:block">Turn this off for an outage or anything nobody should be able to hide.</span>

            <div class="form-acts">
              <button class="btn primary" id="saveBtn" onclick="saveAnnouncement()">Save announcement</button>
...
<script>
  ...
  /* =====================================================================
     ...
     The strip across the top of the dashboard. Everything here talks to
     api/announcement-*.php; nothing is kept client-side beyond the page
     currently on screen, so two managers working at once see the same
     truth after any action.
     ===================================================================== */
  let UCP_NAME="";
  let ITEMS=[], TOTAL_PAGES=1, curPage=1, LOADING=false;

  /* Solid glyphs — kept byte-identical to the dashboard's ANN_ICONS so the
     preview cannot drift from what players see. */
  const ICONS_ANN={
  ...
  /* The strip itself — used for the preview, the "live now" panel and the
     dashboard. One function, so the three can't drift apart. */
  /* The strip itself — used for the preview and the "live now" panel, and
     kept identical to the dashboard's own renderer so the preview can't
     quietly become a lie. */
  function stripHTML(a){
    const t=TYPES[a.type]||TYPES.notice;
    return `<div class="ann ${t.cls}">
      <span class="ann-stamp"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor">${ICONS_ANN[a.type]||ICONS_ANN.notice}</svg></span>
      <div class="ann-main">
        <div class="ann-eyebrow">${escapeHtml(t.label)}<span class="sep">·</span><span class="ago">${escapeHtml(a.at?whenFrom(a.at):'just now')}</span></div>
        <div class="ann-head">${escapeHtml(a.lead)}</div>
        ${a.body?`<div class="ann-detail">${escapeHtml(a.body)}</div>`:''}
      </div>
      <div class="ann-acts">
        ${a.link?`<span class="ann-link">Read more<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M5 12h13M13 6l6 6-6 6"/></svg></span>`:''}
        ${a.dismissable?`<button class="ann-x" type="button" tabindex="-1"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M6 6l12 12M18 6L6 18"/></svg></button>`:''}
      </div>
    </div>`;
  }
  ...
  function rowHTML(a){
    const t=TYPES[a.type]||TYPES.notice;
    return `<div class="arow ${a.active?'live':''}" data-id="${a.id}">
      <div class="abody">
        <div class="aline">
          <span class="tchip">${escapeHtml(t.label)}</span>
          ${escapeHtml(a.lead)}
          ${a.active?`<span class="live-badge"><span class="dot"></span>Live</span>`:''}
        </div>
        ${a.body?`<div class="asub">${escapeHtml(a.body)}</div>`:''}
        <div class="ameta">${escapeHtml(whenFrom(a.at))} · ${escapeHtml(a.by)}${a.dismissable?'':' · not dismissable'}</div>
      </div>
      <div class="aacts">
        <button class="btn sm ${a.active?'ghost':'primary'}" onclick="setActive(${a.id},${a.active?'false':'true'})">${a.active?'Take down':'Publish'}</button>
        <button class="icon-act" title="Edit" onclick="openEditor(${a.id})"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M4 20h4L18 10l-4-4L4 16z"/><path d="M13 5l4 4"/></svg></button>
        <button class="icon-act del" title="Delete" onclick="askDelete(${a.id})"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M5 7h14M9 7V5h6v2M6 7l1 13h10l1-13"/></svg></button>
      </div>
    </div>`;
  }
  ...
  function renderList(){
    ...
    const live=ITEMS.find(a=>a.active);
    document.getElementById('liveBar').innerHTML = live
      ? `<div class="preview-label" style="margin-bottom:9px"><span class="dotlbl"></span>Live on the dashboard now</div>${stripHTML(live)}`
      : `<div class="slotbar" style="margin:0"><span class="cnt" style="color:var(--text-dim);font-weight:600">Nothing is showing on the dashboard.</span></div>`;

    if(!ITEMS.length){
      host.innerHTML=`<div class="empty">
        <span class="ei"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M3 11l14-5v12L3 15v-4z"/><path d="M17 8a3 3 0 0 1 0 8"/></svg></span>
        <h4>No announcements yet</h4>
        <p>Write one for downtime, an event, or anything the whole county needs to see.</p>
        <button class="btn primary" onclick="openEditor()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 5v14M5 12h14"/></svg>New announcement</button>
      </div>`;
  ...
  function renderPager(){
    ...
    let out=`<button class="pg" data-pg="${curPage-1}" ${curPage===1?'disabled':''}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M15 18l-6-6 6-6"/></svg></button>`;
    for(let i=1;i<=TOTAL_PAGES;i++) out+=`<button class="pg ${i===curPage?'on':''}" data-pg="${i}">${i}</button>`;
    out+=`<button class="pg" data-pg="${curPage+1}" ${curPage===TOTAL_PAGES?'disabled':''}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M9 6l6 6-6 6"/></svg></button>`;
    pager.innerHTML=out;
  }
  ...
  function loadPage(n){
    ...
    return UCP.get('announcements.php?scope=all&page='+curPage).then(function(d){
      LOADING=false;
      if(!d || d.ok!==true){
        if(d && d.authenticated===false){ window.location.replace('/login?return='+encodeURIComponent('/dashboard/announcements')); return; }
        toast('Could not load announcements'); return;
      }
      ITEMS=d.announcements||[]; TOTAL_PAGES=d.pages||1; curPage=d.page||curPage;
      renderList();
  ...
  function setActive(id,on){
    UCP.post('announcement-activate.php',{id:id,on:on}).then(function(res){
      const d=res.data||{};
  ...
  function askDelete(id){
    const row=document.querySelector(`.arow[data-id="${id}"]`); if(!row) return;
    const acts=row.querySelector('.aacts');
    if(row.dataset.confirming) return;
    row.dataset.confirming='1';
    const prev=acts.innerHTML;
    acts.innerHTML=`<span class="q" style="font-size:12.5px;color:#e0a99b;font-weight:600">Delete this?</span>
      <button class="btn sm ghost" data-cancel>Cancel</button>
      <button class="btn sm danger" data-confirm>Delete</button>`;
    acts.querySelector('[data-cancel]').onclick=()=>{acts.innerHTML=prev;delete row.dataset.confirming;};
    acts.querySelector('[data-confirm]').onclick=()=>{
      UCP.post('announcement-delete.php',{id:id}).then(function(res){
        const d=res.data||{};
        if(!d.ok){ toast(d.error||'That did not work'); acts.innerHTML=prev; delete row.dataset.confirming; return; }
        toast(d.message); loadPage();
      }).catch(function(){ toast('Could not reach the server'); acts.innerHTML=prev; delete row.dataset.confirming; });
    };
  }

  /* ===================== EDITOR ===================== */
  let editingId=null;
  let draft={type:'notice',lead:'',body:'',link:'',dismissable:true};
  ...
  function showView(name){
    ...
    document.querySelector('.page-title h1').textContent = name==='edit' ? 'Announcements — Editor' : 'Announcements';
    window.scrollTo(0,0);
  }
  ...
  function openEditor(id){
    if(id){
      ...
      editingId=id;
      draft={type:a.type,lead:a.lead,body:a.body||'',link:a.link||'',dismissable:a.dismissable};
      document.getElementById('editTitle').textContent='Edit announcement';
    } else {
      editingId=null;
      draft={type:'notice',lead:'',body:'',link:'',dismissable:true};
      document.getElementById('editTitle').textContent='New announcement';
    }
    document.getElementById('fLead').value=draft.lead;
    document.getElementById('fBody').value=draft.body;
    document.getElementById('fLink').value=draft.link;
    document.querySelectorAll('#typeSeg button').forEach(b=>b.classList.toggle('on',b.dataset.type===draft.type));
    document.getElementById('dismissRow').classList.toggle('on',draft.dismissable);
    document.getElementById('dismissLabel').textContent = draft.dismissable ? 'Players can dismiss this' : 'Players cannot dismiss this';
    showView('edit'); syncPreview();
  }
  function closeEditor(){ showView('list'); }
  ...
  function toggleDismiss(){
    draft.dismissable=!draft.dismissable;
    document.getElementById('dismissRow').classList.toggle('on',draft.dismissable);
    document.getElementById('dismissLabel').textContent = draft.dismissable ? 'Players can dismiss this' : 'Players cannot dismiss this';
    syncPreview();
  }
  ...
  function syncPreview(){
    draft.lead=document.getElementById('fLead').value;
    draft.body=document.getElementById('fBody').value;
    draft.link=document.getElementById('fLink').value;
    document.getElementById('leadCount').textContent=draft.lead.length;
    ...
    document.getElementById('apv').innerHTML=stripHTML({
      type:draft.type,
      lead:draft.lead||'Your headline appears here',
      body:draft.body,
      link:(lc.ok?lc.url:null),
      dismissable:draft.dismissable
    });
    document.getElementById('saveBtn').disabled = !(draft.lead.trim() && (!draft.link.trim() || lc.ok));
  }

  function saveAnnouncement(){
    if(!draft.lead.trim()) return;
    const lc=checkLink(draft.link);
    if(draft.link.trim() && !lc.ok){ toast('Fix the link first'); return; }

    const btn=document.getElementById('saveBtn'), label=btn.textContent;
    btn.disabled=true; btn.textContent='Saving…';
    const payload={type:draft.type,lead:draft.lead.trim(),body:draft.body.trim(),
                   link:lc.url||'',dismissable:draft.dismissable};
    if(editingId) payload.id=editingId;

    UCP.post('announcement-save.php',payload).then(function(res){
      btn.disabled=false; btn.textContent=label;
      const d=res.data||{};
      if(!d.ok){ toast(d.error||'That did not work'); return; }
      toast(d.message); closeEditor(); loadPage(editingId?curPage:1);
  ...
  /* =====================================================================
     BOOT — same gate as the bulletin page. The endpoints enforce it too.
     ===================================================================== */
  function lockOut(){
    document.querySelector('.content').innerHTML=`
      <div class="empty" style="margin:60px auto;max-width:520px">
        ...
        <p>Announcements are managed by Management and Founders. If you think you should have access, speak to a Founder.</p>
  ...
  UCP.get('session.php').then(function(d){
    if(!d || d.authenticated!==true){
      window.location.replace('/login?return='+encodeURIComponent('/dashboard/announcements'));
      return;
    }
    UCP_NAME=d.name||'';
