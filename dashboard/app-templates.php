...
<script>
  ...
  UCP.get('session.php').then(function(d){
    if(!d || d.authenticated !== true){
      window.location.replace('/login?return=' + encodeURIComponent('/dashboard/app-templates'));
      return;
    }
  ...
  /* =====================================================================
     ...
     The same list component as the Question Manager, on purpose: the two
     pages do the same kind of work, and a reviewer who has learned one has
     learned the other.

     Templates ARE deleted rather than retired, unlike questions. What a
     template produced was copied into a feedback field and edited before
     it was sent, so nothing in the archive quotes this table.
     ===================================================================== */
  var ROWS = [], USES = {pass:'Pass', deny:'Deny', either:'Either'}, EDITING = -1, OPEN = {};
  ...
  function meta(t){
    var bits = ['used ' + num(t.used) + ' time' + (t.used===1?'':'s')];
    if(t.by) bits.push('added by ' + escapeHtml(t.by));
    return bits.join(' · ');
  }
  function markClass(t){
    return t.use_for === 'pass' ? 'mark on' : (t.use_for === 'deny' ? 'mark no' : 'mark');
  }

  function itemHTML(t, i){
    var open = !!OPEN[t.id], edit = EDITING === t.id;

    if(edit){
      var opts = Object.keys(USES).map(function(k){
        return '<option value="' + k + '"' + (t.use_for === k ? ' selected' : '') + '>Use for: ' +
               USES[k] + '</option>';
      }).join('');
      return '<div class="item editing" data-id="' + t.id + '"><div class="ihead">' +
        '<span class="grip">' + GRIP + '</span><span class="idx">' + (i+1) + '</span>' +
        '<span class="tx"><b>' + escapeHtml(t.title || 'New template') + '</b><span>editing</span></span>' +
        '<span class="r"><span class="' + markClass(t) + '">' + USES[t.use_for] + '</span></span></div>' +
        '<div class="ibody"><div class="pad">' +
        '<div class="flab">Title</div>' +
        '<input type="text" id="eTitle" value="' + escapeHtml(t.title || '') + '" ' +
          'placeholder="Lack of effort v1">' +
        '<div class="flab">Text</div>' +
        '<textarea id="eBody" rows="7" placeholder="The response itself…">' +
          escapeHtml(t.body || '') + '</textarea>' +
        '<div class="frow">' +
          '<select class="sel" id="eUse" style="width:170px">' + opts + '</select>' +
          (t.id ? '<button class="btn sm" id="eDelete">Delete</button>' : '') +
    ...
    return '<div class="item" data-id="' + t.id + '" draggable="true">' +
      '<div class="ihead" data-toggle="' + t.id + '">' +
      '<span class="grip" data-grip>' + GRIP + '</span><span class="idx">' + (i+1) + '</span>' +
      '<span class="tx"><b>' + escapeHtml(t.title) + '</b><span>' + meta(t) + '</span></span>' +
      '<span class="r"><span class="' + markClass(t) + '">' + USES[t.use_for] + '</span>' +
        '<span class="ib" data-edit="' + t.id + '" title="Edit">' + PEN + '</span>' +
        '<span class="ib" data-toggle="' + t.id + '">' + chev(open) + '</span></span></div>' +
      (open ? '<div class="ibody"><div class="pad"><p>' + escapeHtml(t.body) + '</p></div></div>' : '') +
      '</div>';
  }
  ...
  function render(){
    paintList(
      '<div class="card"><div class="card-h"><h3>Templates</h3><div class="r">' +
      '<button class="btn sm" id="addBtn">New template</button></div></div>' +
      '<div style="padding:16px">' +
      (ROWS.length
        ? '<div class="items" id="list">' + ROWS.map(itemHTML).join('') + '</div>'
        : '<div class="empty">No templates yet. The feedback box still takes free text — ' +
          'these only save typing.</div>') +
      '</div></div>');
    wire();
  }
  ...
  function wire(){
    el('addBtn').addEventListener('click', function(){
      if(!ROWS.some(function(t){ return t.id === 0; }))
        ROWS.unshift({id:0, title:'', body:'', use_for:'deny', used:0, by:null});
      EDITING = 0; render();
    });
    ...
    document.querySelectorAll('[data-edit]').forEach(function(x){
      x.addEventListener('click', function(e){
        e.stopPropagation();
        EDITING = parseInt(this.getAttribute('data-edit'), 10); render();
      });
    });
  ...
  function wireEditor(){
    if(!el('eSave')) return;
    var row = ROWS.filter(function(t){ return t.id === EDITING; })[0];
    if(!row) return;

    el('eCancel').addEventListener('click', function(){
      if(row.id === 0) ROWS = ROWS.filter(function(t){ return t.id !== 0; });
      EDITING = -1; render();
    });
    ...
    if(del) del.addEventListener('click', function(){
      ...
      if(b.getAttribute('data-sure') !== '1'){
        ...
        b.textContent = 'Delete — sure?';
        setTimeout(function(){ b.removeAttribute('data-sure'); b.textContent = 'Delete'; }, 4000);
        return;
      }
      UCP.post('app-template-save.php', {action:'delete', id:row.id}).then(function(res){
        var d = res.data || {};
        if(!d.ok) return toast(d.error || 'Could not delete it');
        EDITING = -1; toast('Deleted.'); load();
      }).catch(function(){ toast('Could not reach the server'); });
    });

    el('eSave').addEventListener('click', function(){
      var b = this; b.disabled = true;
      UCP.post('app-template-save.php', {
        action:'save', id: row.id || 0,
        title: el('eTitle').value, body: el('eBody').value, use_for: el('eUse').value
      }).then(function(res){
        var d = res.data || {};
        b.disabled = false;
  ...
  function saveOrder(){
    ...
    UCP.post('app-template-save.php', {action:'reorder', order: ids}).then(function(res){
      if(res.data && res.data.ok){ toast('Order saved.'); load(); }
    }).catch(function(){ toast('Could not save the new order'); });
  }

  function load(){
    UCP.get('app-templates.php').then(function(d){
      if(!d || !d.ok){
        el('body').innerHTML = '<div class="card"><div style="padding:18px"><p class="lede">' +
          escapeHtml((d && d.error) || 'Could not load the templates.') + '</p></div></div>';
        return;
      }
      ROWS = d.rows; if(d.uses) USES = d.uses; render();
    });
  }
