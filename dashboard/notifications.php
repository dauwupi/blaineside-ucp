...
$PAGE_LEDE = 'Everything the UCP has told you, newest first. Opening one takes you to what it is about; the tick keeps it and the bin throws it away.';
$PAGE_HEAD = <<<'HTML'
<style>
  :root{
...
.imgframe img{position:absolute;inset:0;width:100%;height:100%;user-select:none;-webkit-user-drag:none;object-fit:cover}
  ...
  /* The area badge is a filled glyph; the row's two buttons stay stroked. */
  .nrow .nic svg.f{width:16px;height:16px;fill:currentColor;stroke:none}
...
<script>
  ...
  /* Stroked glyphs for the two controls on a row, and FILLED glyphs for
     the area badge. At 15px on a 34px tile a 2px outline loses its detail
     and every badge collapses into the same grey ring — which is what the
     list looked like when `application` had no entry here at all and fell
     back to the system circle. */
  var NI = {
  ...
  /* One per area, plus an outcome where the outcome is the whole point of
     the notification: an application that passed and one that was denied
     are not the same news and should not wear the same badge. */
  var NAREA = {
  ...
  function areaIcon(n){
    var k = (n.area || '') + '.' + (n.kind || '');
    var d = NAREA[k] || NAREA[n.area] || NAREA.system;
    /* evenodd, so the detail inside a glyph — the ruled lines on the
       document, the tick or cross on a decided application — cuts out of
       the fill instead of merging into it and disappearing. */
    return '<svg class="f" viewBox="0 0 24 24" fill-rule="evenodd">' + d + '</svg>';
  }
  ...
  function nRow(n){
    ...
    return '<div class="nrow'+(n.read ? '' : ' unread')+'" data-id="'+n.id+'"'+
      ...
      '<span class="nic">'+areaIcon(n)+'</span>'+
      '<span class="nb">'+
